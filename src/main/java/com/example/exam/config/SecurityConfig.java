    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable())
                .authorizeHttpRequests((authorize) ->
                        authorize
                                // Public ML APIs (highest priority)
                                .requestMatchers("/public/**").permitAll()
                                .requestMatchers("/api/ai-analytics/**").permitAll()
                                // Public resources
                                .requestMatchers("/js/**","/static/**", "/css/**", "/images/**", "/uploads/**").permitAll()
                                // Public pages
                                .requestMatchers("/", "/login", "/register", "/h2-console/**", "/student/ai-dashboard", "/dashboard").permitAll()
                                // Admin-only pages
